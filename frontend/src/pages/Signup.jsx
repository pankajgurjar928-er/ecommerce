...
const Signup = () => {
  ...
  const handleSignup = async (e) => {
    ...
    let avatar = "https://api.dicebear.com/7.x/avataaars/svg?seed=Milo&backgroundColor=ffd5dc";
    if (gender === "Male") avatar = "https://api.dicebear.com/7.x/notionists/svg?seed=Felix&backgroundColor=b6e3f4";
    if (gender === "Female") avatar = "https://api.dicebear.com/7.x/lorelei/svg?seed=Aneka&backgroundColor=c0aede";
    ...
    try {
      ...
      toast.success(res.data.message || "Identity Established: Welcome to the Studio.");
      navigate("/");
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-white dark:bg-apex-900 font-sans selection:bg-black selection:text-white dark:selection:bg-white dark:selection:text-black overflow-hidden uppercase">
      {/* 🎬 Cinematic Image Side (60% Desktop) - Reversed for Signup */}
      <div className="hidden lg:block lg:w-[60%] relative overflow-hidden order-2">
        <img 
          src="https://images.unsplash.com/photo-1490481651871-ab68de25d43d?q=80&w=2000&auto=format&fit=crop" 
          alt="Signup Aesthetic" 
          className="absolute inset-0 w-full h-full object-cover grayscale-[0.3] hover:grayscale-0 transition-all duration-[5000ms] ease-out scale-110 hover:scale-100"
        />
        <div className="absolute inset-0 bg-gradient-to-l from-transparent to-white dark:to-apex-900" />
        
        {/* Floating Branding Section */}
        <div className="absolute top-20 right-20 z-10 animate-reveal text-right">
          ...
          <div className="flex items-center justify-end mt-6 space-x-6">
            <p className="text-[10px] font-black uppercase tracking-[0.5em] text-black dark:text-white">Studio Membership / 02</p>
            <div className="w-12 h-px bg-black dark:bg-white" />
          </div>
        </div>
      </div>

      {/* 🧾 Signup Form Side (40% Desktop) */}
      <div className="w-full lg:w-[40%] min-h-screen flex flex-col justify-center px-8 sm:px-12 lg:px-20 py-20 bg-white dark:bg-apex-900 z-20 order-1">
        <div className="max-w-md w-full mx-auto animate-slide-up">
          ...
          <div className="mb-14">
            ...
            <p className="text-slate-400 text-xs font-black uppercase tracking-[0.2em] leading-relaxed">
              Join the elite circle of connoisseurs.
            </p>
          </div>
          ...
          <form onSubmit={handleSignup} className="space-y-6">
            ...
            <div className="space-y-4">
              <label className="block text-[8px] font-black text-slate-400 uppercase tracking-[0.3em] mb-2 px-1">
                Vessel Identity
              </label>
              <div className="grid grid-cols-3 gap-3">
                {["Male", "Female", "Other"].map((g) => (
                  <button
                    ...
                    type="button"
                    onClick={() => setGender(g)}
                    className={`py-3 text-[9px] font-black uppercase tracking-[0.2em] border transition-all ${gender === g ? 'bg-black text-white border-black dark:bg-white dark:text-black border-white' : 'border-black/10 dark:border-white/10 text-slate-400 hover:border-black/30 dark:hover:border-white/30'}`}
                  >
                    {g}
                  </button>
                ))}
